...
cost cutoff or an travel cost interval), minimum travel cost to closest N
...
1. `cost_to_closest()` - calculates the mininimum travel cost to the closest
   *n* number of opportunities.
2. `cumulative_cutoff()` - calculates the frenquently used threshold-based
...
monetary costs, risk perception, etc., can be considered a generic *cost*.

The `gravity()` and `floating_catchment_area()` functions can use
different decay functions when estimating accessibility levels. This decay
...
`id` column, which refer to the ids listed in the travel matrix, and the number
...
(*forthcoming paper*), mitigates the impacts of arbitrary choices of cost
...
them with a range of possible different parameters. In order to accomodate such
...
`fca_metric` parameter. As of the time of writing this vignette, the package
...
competing for opportunities with the `competition_col` parameter. The function
also support diferent decay functions. In the example below, we calculate
...
The example belows quickly shows how to create a simple map using `{ggplot2}`.
